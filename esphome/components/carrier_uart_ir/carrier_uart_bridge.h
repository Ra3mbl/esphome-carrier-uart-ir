...
class CarrierUartBridge : public climate::Climate,
 ...
 protected:
  ...
  // ===== ƒекодирование режима/температуры =====
  ...
  void decode_mode_block_(const uint8_t *m) {
    ...
    float temp = this->decode_temperature_(b0, b1);
    if (!std::isnan(temp))
